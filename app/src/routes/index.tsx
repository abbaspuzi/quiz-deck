import { useState } from 'react'
import { useNavigate, createFileRoute } from '@tanstack/react-router'
import { useQuery } from 'convex/react'
import { api } from '../../convex/_generated/api'
import { authClient } from '../lib/auth-client'
import { quizModes } from '../features/quiz/content'
import type { QuizMode } from '../features/quiz/content'

function IndexPage() {
  const navigate = useNavigate({ from: '/' })
  const user = useQuery(api.auth.getCurrentUser)
  const [selectedMode, setSelectedMode] = useState<QuizMode>('all')

  const handleGoogleSignIn = () => {
    void authClient.signIn.social({
      provider: 'google',
      callbackURL: '/',
    })
  }

  const handleSignOut = () => {
    void authClient.signOut()
  }

  const handleStartQuiz = () => {
    const name = user?.name ?? 'Guest'
    void navigate({
      to: '/quiz/$sessionId',
      params: { sessionId: `s-${Date.now()}` },
      search: { name, mode: selectedMode === 'all' ? undefined : selectedMode },
    })
  }

  return (
    <div className="start-screen">
      <div className="flashcard">
        <div className="card-info">
          <h2 className="headline">Mother&apos;s Day Product Quiz</h2>
          <p className="subheadline">
            Wrapper color, hero count, sunflower heart color, ribbon hardware, and
            filler mistakes that should never escape QC.
          </p>

          <div className="login-form">
            {user ? (
              <>
                <div className="welcome-back">
                  {user.image && (
                    <img
                      src={user.image}
                      alt=""
                      className="avatar"
                    />
                  )}
                  <span>Welcome, <strong>{user.name}</strong></span>
                </div>
                <div className="quiz-mode-selector">
                  {quizModes.map((mode) => (
                    <button
                      key={mode.id}
                      type="button"
                      className={`mode-option${selectedMode === mode.id ? ' active' : ''}`}
                      onClick={() => setSelectedMode(mode.id)}
                    >
                      <strong>{mode.label}</strong>
                      <span>{mode.description}</span>
                    </button>
                  ))}
                </div>
                <button className="button" type="button" onClick={handleStartQuiz}>
                  Start quiz
                </button>
                <div className="cta-row" style={{ justifyContent: 'center' }}>
                  <button
                    className="ghost-button"
                    type="button"
                    onClick={() => void navigate({ to: '/dashboard' })}
                  >
                    Dashboard
                  </button>
                  <button className="ghost-button" type="button" onClick={handleSignOut}>
                    Sign out
                  </button>
                </div>
              </>
            ) : (
              <>
                <button className="google-btn" type="button" onClick={handleGoogleSignIn}>
                  <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
                    <path
                      fill="#4285F4"
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                    />
                    <path
                      fill="#34A853"
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    />
                    <path
                      fill="#EA4335"
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    />
                  </svg>
                  Sign in with Google
                </button>
                <button
                  className="ghost-button"
                  type="button"
                  onClick={() => void navigate({ to: '/dashboard' })}
                >
                  Leaderboard
                </button>
              </>
            )}
          </div>
        </div>
      </div>
    </div>
  )
}

export const Route = createFileRoute('/')({
  component: IndexPage,
})
